...
public class MiniMessageConverter {
    public static Component getFromLegacy(@NotNull String legacyString, @NotNull TagResolver... resolvers) {
        return MiniMessage.miniMessage().deserialize(MiniTranslator.toMini(legacyString), resolvers);
    }
    ...
    public final class MiniTranslator {
        ...
        public static @NotNull String toMini(@NotNull String text, @NotNull Collection<@NotNull Option> options) {
            ...
            if (closeLastColor || !fastReset)
                handleClosing(order, builder, closeLastColor, closeLastColor && fastReset);
